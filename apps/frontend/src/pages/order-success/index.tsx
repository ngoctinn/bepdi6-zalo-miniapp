...
export default function OrderSuccessPage() {
  ...
  const handleViewOrder = () => {
    if (orderId) {
      navigate(`/order/${orderId}`, {
        state: state?.order ? { order: state.order } : undefined,
      });
    } else {
      navigate("/order");
    }
  ...
  return (
    <div className="flex h-full min-h-screen flex-col items-center justify-center bg-background px-6 pb-28 text-center">
      ...
      <div className="safe-bottom fixed bottom-0 left-0 right-0 border-t border-black/5 bg-background/95 px-4 pb-3 pt-3 shadow-lg backdrop-blur-md">
        <div className="mx-auto flex max-w-md gap-3">
          <button
            type="button"
            onClick={() => navigate("/")}
            className="flex-1 rounded-xl border border-black/10 bg-white py-3 text-sm font-semibold text-neutral800 transition-all active:scale-[0.98] active:bg-black/5"
          >
          ...
          <button
            ...
            className="shadow-xs flex-1 rounded-xl bg-primary py-3 text-sm font-bold text-white transition-all active:scale-[0.98] active:bg-primaryDark"
          >
