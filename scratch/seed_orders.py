...
from apps.menu.models import Product, Option, OptionGroup
...
def seed():
    ...
    p_bun_bo = next((p for p in products if "Bún" in p.name), products[min(1, len(products)-1)])
    p_tra_tac = next((p for p in products if "Trà" in p.name), products[min(2, len(products)-1)])
    ...
    opt_trung, _ = Option.objects.get_or_create(option_group=grp1, name="Trứng ốp la", defaults={"price": Decimal("5000")})
    opt_cha, _ = Option.objects.get_or_create(option_group=grp1, name="Chả hấp", defaults={"price": Decimal("7000")})
    opt_suon, _ = Option.objects.get_or_create(option_group=grp1, name="Sườn thêm", defaults={"price": Decimal("20000")})
    ...
    opt_bap_bo, _ = Option.objects.get_or_create(option_group=grp2, name="Thêm bắp bò", defaults={"price": Decimal("15000")})
    opt_cha_cua, _ = Option.objects.get_or_create(option_group=grp2, name="Chả cua", defaults={"price": Decimal("10000")})
    ...
    opt_tran_chau, _ = Option.objects.get_or_create(option_group=grp3, name="Trân châu trắng", defaults={"price": Decimal("5000")})
    opt_nha_dam, _ = Option.objects.get_or_create(option_group=grp3, name="Thạch nha đam", defaults={"price": Decimal("5000")})
    ...
    for data in sample_orders:
        ...
        Payment.objects.create(
            order=order,
            method=Payment.Method.BANK_TRANSFER if data["payment_method"] == Order.PaymentMethod.BANK_TRANSFER else Payment.Method.COD,
            status=data["payment_status"],
            amount=total_amount,
            actual_paid_amount=total_amount if data["payment_status"] == Payment.Status.PAID else Decimal("0"),
            paid_at=timezone.now() if data["payment_status"] == Payment.Status.PAID else None
        )

        print(f"-> Đã tạo đơn #{order.order_code} | {order.get_status_display()} | {order.recipient_name} | {total_amount:,.0f}đ")
